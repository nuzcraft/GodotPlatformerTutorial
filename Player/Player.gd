...
func fast_fall(delta):
	if velocity.y > 0:
		velocity.y += moveData.FORCE_FALL_GRAVITY*delta
		animatedSprite.animation = "Falling"
	else:
...
func apply_acceleration(amount, delta):
	velocity.x = move_toward(velocity.x, moveData.HORIZONTAL_MAX_SPEED*amount, moveData.HORIZONTAL_ACCELERATION * delta)
